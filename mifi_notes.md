...
research_base = {
    "python_01": {"name": "Каа", "age": 5, "health": "Perfect"},
    "boa_02": {"name": "Нагайна", "age": 2, "health": "Perfect"},
    "cobra_03": {"name": "Шесс", "age": 4, "health": "Weak"}
}

adult_snakes = {key for key, data in research_base.items() if data.get("age", 0) > 3}
perfect_health_snake = {key for key, data in research_base.items() if data.get("health", "None") == "Perfect"}
adult_perfect = adult_snakes & perfect_health_snake
print(f'взрослее 3 лет и с отличным состоянием: {adult_perfect}')

Теория кода
множество (set) — это неупорядоченная коллекция уникальных элементов, нет повторений.

Основные функции.

Пересечение (& или .intersection()): Что есть и там, и там.
Пример: Список змей в линьке И список змей, которых пора кормить. На выходе — те, кого кормить опасно (из-за линьки).
Объединение (| или .union()): Всё вместе без повторов.
Пример: Все змеи из террариума №1 и террариума №2 в один общий список.
Разность (- или .difference()): Что есть в первом, но нет во втором.
Пример: Список всех змей МИНУС список змей, которые уже поели. На выходе — те, кто остался голодным.
